...
public class XmlTarefasArquivadas extends Xml implements XmlInterface{
    ...
    public XmlTarefasArquivadas(Context contexto) {
        super(contexto);
        setNomeArquivoXML();
    }
    ...
    public static boolean criaXmlTarefasArquivadasWebservice(Usuario usuario, boolean forcarAtualizacao) throws IOException {
        ...
        String xml = webService.tarefasArquivadas();
        if (xml != null) {
            escreveXML(xml);
            return true;
        }else
     ...
     * @throws IOException
     */
    public void criaXmlTarefasArquivadasWebservice(String xml) throws IOException {
        escreveXML(xml);
    }
